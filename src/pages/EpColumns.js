...
const EpColumns = ({ translateStore }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      const data = await translateStore.get_TRREPCLMNS({ top, skip });
        ...
        {isLoading ? (
          ...
          <>
            <div
              ...
              <Title>תרגום לעמודות של משהומשהו</Title>
            </div>
            <ul style={{ padding: 0 }}>
              <li>
                <div
              ...
              {currentItems?.map((item, index) => {
                let translationValue = !!item?.LANGREPCLMNS_SUBFORM?.length
                  ? item?.LANGREPCLMNS_SUBFORM?.find((it) => it.LANG === 2)
                      ?.TITLE
                  : "";
                return (
                  <li
...
export default inject("translateStore")(observer(EpColumns));
